...
class Ubareports_model{
    ...
    function lastSvt() {
     ...
     function pieChartUBA() {
    	//$this->prepararConsultaUsuario('opc_pieChartSVT');  

    	$jsonArray = array();
		$consultaSql = "call sp_uba_report('opc_pieChartUBA','".$this->param['param_fecha1']."','".$this->param['param_fecha2']."')";
		//echo $consultaSql;
		$this->result3 = mysqli_query($this->conexion,$consultaSql);

    	  while($row = $this->result3->fetch_assoc()) {
		    $jsonArrayItem = array();
		    $jsonArrayItem['horas'] = $row['horas'];
		    $jsonArrayItem['operaciones'] = $row['operaciones'];
	...
	function lastIn() {
    	...
    	while($row = mysqli_fetch_row($this->result)){
    		...
    		switch($row[3])
				{
			case "Analista";
				$row3='<span class="label label-info">'.$row[3].'</span>';
				break;
			case "Visitante";
				$row3='<span class="label label-warning">'.$row[3].'</span>';
				break;
			default :
				$row3='<span class="label label-success">'.$row[3].'</span>';
			}
