...
function mostrarDatos() {
    $.ajax({
        type: 'POST',
        data: {param_opcion: 'listar_networker'},
        url: '../../controller/bitacoras/networker_controller.php',
        success: function (respuesta) {
            $('#table_networker').DataTable().destroy();
            $('#body_networker').html(respuesta);
            $('#table_networker').DataTable({
                'paging': true,
                'lengthChange': true,
                'searching': true,
                'ordering': true,
